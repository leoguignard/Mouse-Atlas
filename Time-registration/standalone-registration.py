...
def produce_trsf(params):
    ...
    if not os.path.exists(p_out + 't%03d-%03d.txt'%(t1, t2)) or os.path.exists(p_out + 't%03d-%03d.txt'%(t2, t1)):
        ...
        if t1 < reference_time:
            os.system('blockmatching -ref ' + out2 + ' -flo ' + out1 +\
                      ' -res tmp.mha -trsf-type translation -py-hl 6 -py-ll 2' + \
                      ' -res-voxel-trsf ' + p_out + 't%03d-%03d.txt'%(t1, t2))
        else:
            os.system('blockmatching -ref ' + out1 + ' -flo ' + out2 +\
                      ' -res tmp.mha -trsf-type translation -py-hl 6 -py-ll 2' + \
...
def compose_trsf(flo_t, ref_t, trsf_p, tp_list):
    ...
    if not os.path.exists(out_trsf):
        ...
        os.system('composeTrsf ' + out_trsf + ' -trsfs ' + trsf_2 + ' ' + trsf_1)
    return out_trsf
...
def read_param_file():
    ...
    return (paths, reference_times, Xs, times, VS, OR, time_list, suffixes,
            registration_methods, registered_folder, comp_trsfs, app_trsfs, TP_to_keep)
...
if __name__ == '__main__':
    ...
    (paths, reference_times, Xs, times, VS, OR, time_list,
        ...
        comp_trsfs, app_trsfs, TP_to_keeps) = read_param_file()
    ...
    for (p, r, X, nb_times, vs, ors,
         ...
         app_trsf, TP_to_keep) in zip(paths, reference_times,
                                      Xs, times, VS, OR, time_list,
                                      folder_names, registration_methods,
                                      registered_folders, comp_trsfs, app_trsfs, TP_to_keeps):
        first_TP = tp_list[0]
        ...
        if registered_folder != [] and app_trsf:
            ...
            p_im = '/data/registration-workshop/TMP/' + p.split('/')[-1].replace('.klb', '.inr')
